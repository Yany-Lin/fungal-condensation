...
# HANDOFF is an internal mirror; if the user-specific path doesn't exist, fall
# back to a sibling _handoff/ next to this script so reviewers can still run it.
_HANDOFF_DEFAULT = Path('/Users/yany/Downloads/manuscript_handoff')
HANDOFF = _HANDOFF_DEFAULT if _HANDOFF_DEFAULT.parent.exists() else Path(__file__).resolve().parent / '_handoff'
...
fig, axes = plt.subplots(1, 4, figsize=(190*MM, 55*MM))
fig.subplots_adjust(left=0.08, right=0.98, top=0.86, bottom=0.18, wspace=0.55)
...
fig = plt.figure(figsize=(190*MM, 55*MM))
gs = fig.add_gridspec(1, 2, left=0.07, right=0.97, top=0.86, bottom=0.18,
...
ax_g.set_ylabel('Asp-favored ratio', fontsize=7); ax_g.set_ylim(0, max(ar)*1.22)
...
ax.text(0.02, 0.95, 'High CV = mix of solid cores\n(low tubeness) + filamentous\nedges (high tubeness)',
        transform=ax.transAxes, fontsize=6, va='top', color=C_ASP,
        bbox=dict(boxstyle='round,pad=0.3', facecolor='white', alpha=0.8))
ax.text(0.50, 0.95, 'Low CV = uniformly\nfilamentous tissue',
        transform=ax.transAxes, fontsize=6, va='top', color=C_MUC,
        bbox=dict(boxstyle='round,pad=0.3', facecolor='white', alpha=0.8))
for sp in ['top','right']: ax.spines[sp].set_visible(False)
